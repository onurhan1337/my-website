...
# Ensure we can print debugging information
import sys
import traceback

try:
    # Redirect output to capture any potential hidden errors
    ...
    # Expanded list of words to provide more samples
    words = [
    ...
    with torch.no_grad():
        ...
        # Average token embeddings for each word
        word_embeddings = []
        for i in range(len(words)):
            # Get the actual tokens for the word (excluding padding)
            word_tokens = inputs.input_ids[i]
            word_tokens = word_tokens[word_tokens != tokenizer.pad_token_id]

            # Average embeddings for this word's tokens
    ...
    # Standardize the embeddings (important for t-SNE)
    ...
    tsne = TSNE(
        n_components=2,
        random_state=42,
        perplexity=min(5, len(words)-1)  # Ensure perplexity is valid
    )
    ...
    plt.show()  # Add this to force display

except Exception as e:
    # Restore stdout and print full error
    sys.stdout = old_stdout
    print(f"Unexpected error: {e}")
    traceback.print_exc()

# Check if debug file exists and print its contents
try:
