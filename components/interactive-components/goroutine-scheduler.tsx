  ...
  return (
    <div className="p-6 bg-white rounded-2xl shadow-sm border border-gray-100">
      ...
      <div className="mb-4 p-3 bg-blue-50 rounded-lg">
        <p className="text-sm text-blue-700 font-medium">{currentDescription}</p>
      </div>

      {/* Global Run Queue */}
      <div className="mb-6">
        <h4 className="text-sm font-medium text-gray-700 mb-2">Global Run Queue</h4>
        <div className="flex gap-2 p-3 bg-gray-50 rounded-lg overflow-x-auto min-h-[4rem] items-center">
          <AnimatePresence mode="popLayout">
            {getGoroutinesByLocation('global').map(g => (
              <motion.div
                ...
                className="flex-shrink-0 w-8 h-8 bg-blue-100 text-blue-700 rounded-lg flex items-center justify-center text-sm font-medium shadow-sm hover:shadow-md transition-shadow"
              >
                G{g.id}
              </motion.div>
            ))}
      ...
      {/* Processors */}
      {[1, 2].map(pid => (
        <div key={pid} className="mb-4 border border-gray-100 rounded-xl p-4">
          <div className="flex items-center justify-between mb-3">
            <div className="flex items-center gap-2">
              <span className="text-sm font-medium text-gray-700">P{pid}</span>
            ...
            <AnimatePresence mode="popLayout">
              {getGoroutinesByLocation(`running-p${pid}` as Goroutine['location']).map(g => (
                <motion.div
                  ...
                  className="px-2 py-1 bg-green-100 text-green-700 rounded-md text-sm font-medium shadow-sm"
                >
                  Running G{g.id}
                </motion.div>
              ))}
          ...
          {/* Local Run Queue */}
          <div className="flex gap-2 p-2 bg-gray-50 rounded-lg overflow-x-auto min-h-[3rem] items-center">
            <AnimatePresence mode="popLayout">
